...
**Economics addendum (2026-06-10, Google published rates for gemini-3.1-flash-live-preview, ai.google.dev/gemini-api/docs/pricing):** audio input $0.005/min ($0.30/hr open mic), video input $0.002/min ($0.12/hr camera), native audio out $0.018/min, text out $4.50/1M (~$0.001/reply). ElevenLabs Flash ~5-8 cents/spoken minute on the existing sub. A Claude agent run dominates: ~$1/turn Opus, ~$0.15-0.30 Sonnet, i.e. the brain is 90-95% of spend in EITHER architecture. The cascade's only real economic win is always-on ambient listening ($0 silent vs ~$7/day for a 24/7 open Live session); mitigate by gating the session behind tab-open now and wake word in Phase 3, not by picking the cascade. Dom requirement captured 2026-06-10: **Claude stays the primary brain.** Gemini Live is ears/eyes/receptionist only; its system prompt must force delegation of all substantive asks to Claude (NON_BLOCKING dispatch) and pure relay of Claude's answers; two-personality drift is the main non-cost risk. Gemini's native voice is marginally cheaper than Kenn (~$3-4 per 100 turns); irrelevant at our volume.

**Key status:** Dom minted a fresh Gemini key 2026-06-10 (vault item "Gemini API Key" updated, suffix QbJw), verified against the Live model list same day.
